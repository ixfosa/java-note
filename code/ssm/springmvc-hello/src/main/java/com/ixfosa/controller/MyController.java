...
public class MyController {
     ...
     */
    /**
     * 准备使用doSome方法处理some.xifosa请求。
     * @RequestMapping: 请求映射，作用是把一个请求地址和一个方法绑定在一起。
     *                  一个请求指定一个方法处理。
     *       属性： 1. value 是一个String，表示请求的uri地址的（some.ixfosa）。
    ...
    @RequestMapping(value = {"/some.ixfosa", "/first.ixfosa"})
    public ModelAndView doSome() {
        //处理some.xifosa请求了。 相当于service调用处理完成了。
        ModelAndView mv = new ModelAndView();

        // 添加数据， 框架在请求的最后把数据放入到request作用域。
        //request.setAttribute("msg","欢迎使用springmvc做web开发");
        mv.addObject("msg","hello spring-webmvc");
    ...
    @RequestMapping(value = {"/other.ixfosa","/second.ixfosa"})
